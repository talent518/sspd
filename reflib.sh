...
#Berkeley DB
if [ ! -f "$INST_DIR/include/db.h" ] && [ ! -f "/usr/include/db.h" ]; then
...
#openldap
if [ ! -f "$INST_DIR/include/ldap.h" ] && [ ! -f "/usr/include/ldap.h" ]; then
...
if [ ! -d "$INST_DIR/lib64" ] && [ `uname -p` = "x86_64" ]; then
...
if [ ! -f "$INST_DIR/lib/libphp5.so" ]; then
    echo Installing php ...
    if [ ! -d "/tmp/php-5.6.30" ]; then
        tar -xvf php-5.6.30.tar.bz2 -C /tmp/
    fi
    pushd /tmp/php-5.6.30
