...
class LibImageVerify extends LibImageBase {
	...
	function enVerify ( $length = 4, $mode = 1, $type = 'png', $width = 180, $height = 50, $fontface = 'simhei.ttf' ) {
		$randval = LIB('string')->rand($length, $mode);
		LIB('cookie')->set('verify', $randval);
	...
	function cnVerify ( $length = 4, $type = 'png', $width = 180, $height = 50, $fontface = 'simhei.ttf' ) {
		$code = LIB('string')->rand($length, STRING_RAND_CN, CFG()->charset);
		$width = ( $length * 45 ) > $width ? $length * 45 : $width;
		LIB('cookie')->set('verify', $code);
		$im = imagecreatetruecolor($width, $height);
		$borderColor = imagecolorallocate($im, 100, 100, 100); // 边框色
		...
		for ( $i = 0; $i < $length; $i ++  ) {
			$fontcolor = imagecolorallocate($im, mt_rand(0, 120), mt_rand(0, 120), mt_rand(0, 120)); // 这样保证随机出来的颜色较深。
			$codex = LIB('string')->cut($code, $i, 1, CFG()->charset, '');
